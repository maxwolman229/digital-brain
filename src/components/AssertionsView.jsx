        ...
        {sel && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
              ...
              <button onClick={() => { setSel(null); onViewInGraph?.('assertion', sel.id) }} style={{ ...btnStyle, border: '1px solid var(--md1-accent)', color: 'var(--md1-accent)' }}>View in Graph</button>
            </div>
            ...
            <PhotosSection
              photos={sel.photos || []}
              onAdd={async (file) => {
                const url = await uploadPhoto(file, 'assertion', sel.id)
                const updated = { ...sel, photos: [...(sel.photos || []), url] }
                await updateAssertion(sel.id, { title: sel.title, category: sel.category, processArea: sel.processArea, scope: sel.scope, status: sel.status, tags: sel.tags, photos: updated.photos, changeNote: 'Added photo' })
                setSel(updated)
                setAssertions(prev => prev.map(a => a.id === sel.id ? updated : a))
              }}
              onRemove={async (url) => {
                const updated = { ...sel, photos: (sel.photos || []).filter(p => p !== url) }
                await updateAssertion(sel.id, { title: sel.title, category: sel.category, processArea: sel.processArea, scope: sel.scope, status: sel.status, tags: sel.tags, photos: updated.photos, changeNote: 'Removed photo' })
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <PhotosSection
        photos={photos}
        onAdd={async (file) => {
          const url = await uploadPhoto(file, 'assertion', item.id)
          const next = [...photos, url]
          setPhotos(next)
          await updateAssertion(item.id, { title: form.title, category: form.category, processArea: form.processArea, scope: form.scope, status: form.status, tags: form.tagsInput.split(',').map(t => t.trim()).filter(Boolean), photos: next, changeNote: 'Added photo' })
        }}
        onRemove={async (url) => {
          const next = photos.filter(p => p !== url)
          setPhotos(next)
          await updateAssertion(item.id, { title: form.title, category: form.category, processArea: form.processArea, scope: form.scope, status: form.status, tags: form.tagsInput.split(',').map(t => t.trim()).filter(Boolean), photos: next, changeNote: 'Removed photo' })
          await deletePhoto(url)
        }}
      />
...
function AddAssertionForm({ onClose, onCreated, processAreas = [], categories = [], plantId }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <Field label="Detail" hint="Step-by-step instructions, context, conditions — anything beyond the concise title">
        <textarea value={form.scope} onChange={e => set('scope', e.target.value)} rows={4} placeholder="e.g. Observed across shredded grade heats — conditions include X, Y, Z…" style={{ ...iS, resize: 'vertical', whiteSpace: 'pre-wrap' }} />
      </Field>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          {(item.linkedRules || []).length > 0 && (
            <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 2, background: '#f0eeec', color: 'var(--md1-accent)', fontWeight: 600, fontFamily: FNT }}>
